...
We summarize the steps of our method in Algorithm 1, which are explained further in the following subsections.
 ...
 \begin{algorithm}[!h]
 	...
 	\begin{algorithmic}[1]
        ...
        \State $\pi({\mathbf{s})}$=\mathbf{Associate}$(F_l,R^p)$ \% $\pi({\mathbf{s}})$:closest reference point 
        \State \mathbf{Selection}($F_l,R^p,\pi,P_{t+1}$,N)  \% Select Remaining points from last front
